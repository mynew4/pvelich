...
class flask_of_blight : public ItemScript
{
public:
    flask_of_blight() : ItemScript("flask_of_blight") { }

    bool OnUse(Player* player, Item* /*item*/, SpellCastTargets const& /*targets*/)
    {
        if (player->GetQuestStatus(12206) == QUEST_STATUS_INCOMPLETE)
        {
            if (Creature* caged = player->FindNearestCreature(27349, 15.0f, true))
            {
                player->KilledMonsterCredit(27253, 0);
                caged->DealDamage(caged, caged->GetHealth());
            }
        }
        return false;
    }
