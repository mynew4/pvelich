...
public:
    ...
    struct npc_steam_ragerAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* killer)
        {
            ...
            if (player->GetQuestStatus(11893) == QUEST_STATUS_INCOMPLETE)
            {
                if (me->FindNearestCreature(25987, 20, true))
                    player->KilledMonsterCredit(25987, 0);
            }
        }
    };
...
class item_reinforced_net : public ItemScript
{
public:
    item_reinforced_net() : ItemScript("item_reinforced_net") { }

    bool OnUse(Player* player, Item* /*item*/, SpellCastTargets const& targets)
    {
        if (targets.GetUnitTarget() && targets.GetUnitTarget()->GetTypeId() == TYPEID_UNIT && targets.GetUnitTarget()->GetEntry() == 25582)
        {
            player->DealDamage(targets.GetUnitTarget(),targets.GetUnitTarget()->GetMaxHealth());
        }
