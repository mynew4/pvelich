...
int32 PriceInGold = sWorld->getIntConfig(TRANSMOGRIFICATION_REQUIRED_MONEY) * GOLD;
...
class npc_transmogrify : public CreatureScript
{
    public:
        ...
        {
            pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "Поменять!", GOSSIP_SENDER_MAIN, ACTION_TRANSMOGRIFY_ADD_DISPLAY);
            pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "Сбросить.", GOSSIP_SENDER_MAIN, ACTION_TRANSMOGRIFY_REMOVE_DISPLAY);
        ...
        void TransmogrifyItem(Player* player, Creature* creature)
        {
            ...
            switch (result)
            {
                ...
                case FAKE_ERR_SUBCLASS:
                    handler.PSendSysMessage("Items has different subclass!");
                    break;
                case FAKE_ERR_OK:
                {
                    WorldPacket data;
                    ...
                    player->ModifyMoney(-PriceInGold);
