...
class AreaTrigger_at_drake_hunt : public AreaTriggerScript
{
    public:
        ...
        bool OnTrigger(Player* player, AreaTriggerEntry const* trigger)
        {
            Creature* drake = player->FindNearestCreature(NPC_DRAKE, 50);
            if (player->GetQuestStatus(QUEST_DRAKE_HUNT) == QUEST_STATUS_INCOMPLETE && player->HasAura(SPELL_DRAKE_HATCHLING_SUBDUED) && drake)
            {
                player->KilledMonsterCredit(26175, 0);
