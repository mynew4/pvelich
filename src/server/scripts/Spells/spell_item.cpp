...
class spell_lament_of_the_highborne : public SpellScriptLoader
{
    public:
        spell_lament_of_the_highborne() : SpellScriptLoader("spell_lament_of_the_highborne") { }

        class spell_lament_of_the_highborne_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_lament_of_the_highborne_SpellScript);
            ...
            void HandleScript(SpellEffIndex effIndex)
            {
                ...
                if (player)
                {
                    std::list<Player*> playerList = player->GetNearestPlayersList(50.0f);
                    for (std::list<Player*>::const_iterator itr = playerList.begin(); itr != playerList.end(); ++itr)
                            (*itr)->PlayDirectSound(10896, (*itr));
                }
            }

            void Register()
            {
                OnEffectHitTarget += SpellEffectFn(spell_lament_of_the_highborne_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_lament_of_the_highborne_SpellScript();
        }
};
    ...
    public:
        ...
        class spell_toss_your_luck_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex effIndex)
            {
                ...
                handler.PSendSysMessage("%s небрежно подкидывает в воздух свою титановую печать даларана.", caster->GetName());

                switch(side)
                {
                    case 0:
                        handler.PSendSysMessage("%s обнаруживает, что монета упала орлом!",caster->GetName());
                        break;
                    case 1:
                        handler.PSendSysMessage("%s обнаруживает, что монета упала решкой!",caster->GetName());
                        break;
                    default:
                        break;
                }
...
void AddSC_item_spell_scripts()
{
    ...
    new spell_lament_of_the_highborne();
